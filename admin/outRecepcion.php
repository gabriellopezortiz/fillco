...
$semana = date("W");
 $codMalla = '';
 if (isset($_GET['codMalla'])) {

    $codMalla = $_GET['codMalla'];
...
<html lang="en">
    <head>
        ...
        <script type="text/javascript" src="appSalidas.js"></script>-->
    ...
    <?php 
        function CrearCodAsignado(){
            ...
            CrearCodAsignado();	
                ...
                SELECT   *  FROM inventario, variedad_flor WHERE inventario.cod_malla  = '".$codMalla."' AND inventario.codigo_variedad = variedad_flor.codigo_variedad AND inventario.fecha_granel = '".$fechaHoy."' AND  inventario.status= 2 ORDER BY inventario.id_inventario DESC LIMIT 1
                ...
                SELECT count(inventario.id_inventario) as numeroMallas  FROM inventario WHERE  inventario.fecha_granel = '".$fechaHoy."' AND  inventario.status= 2


                 ";

                 $ResulsecondQuery = mysqli_query($conexion, $secondQuery);
                
                if($rowNumber = mysqli_fetch_array($ResulsecondQuery)){

                 $numeroMallas=$rowNumber['numeroMallas']; 
                        ...
                        if (isset($_GET['varError']) == 1){
                                          ...
                                          echo '<style type="text/css">
                                                #cod_malla{
                                                                  ...
                                                                  echo 'alert("Ya ingreso esta malla");
    ...
    <form class="signup-form" name="salida" id="salida" action="procesarSalidaMalla.php" method="post" >
                                    <div class="form-group">
                                        <input id = "fechaactual" name = "fechaactual" type="text" class="form-control" value="<?php echo "Mes: ".$mesCompleto." semana: ".$semana; ?>" readonly  size="40" maxlength="30" style="text-transform:uppercase" required >
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        <input id = "cod_malla" name = "cod_malla" type="text" placeholder="CODIGO MALLA" class="form-control" autofocus size="6" maxlength="6"  required >
                                    </div>
                                  ...
                                  <form class="signup-form" name="cont" id="cont">
                                     <div class="form-group">
                                        <input  id="mallaSacada" name="mallaSacada" type="text" class="form-control" placeholder="MALLA RETIRADA" size="100" maxlength="100" value="<?php 

                                                if (isset($cod_malla)) {echo $cod_malla;} 
                                                ?>">
                                    </div>
                                    <div class="form-group">
                                        <input  id="variedadSacada" name="variedadSacada" type="text" class="form-control" placeholder="VARIEDAD" size="11" maxlength="11"  value="<?php if (isset($nombre)) {echo $nombre;} ?>" >
                                    </div>
                                   
                                
                                   <div class="form-group">
                                        <input  class="btn btn-dark btn-block" type="button" id="valorCuenta" name="valorCuenta"  size="30" maxlength="30" value=" Número de mallas leidas= <?php if (isset($numeroMallas)) {echo $numeroMallas;} ?>">
                                    </div>
        ...
        <audio id = "sonido">
    <source src = "perder-incorrecto-no-valido.mp3" type = "audio/mpeg" />
</audio>
